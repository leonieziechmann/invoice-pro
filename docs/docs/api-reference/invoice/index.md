...
| `order-nr`               | `none` \| `str` \| `content`                                                                 | Order / purchase order number, printed by `references.order-nr()`; in Factur-X / ZUGFeRD, BT-13. See [Document References](../../e-invoicing.md#8-document-references).                                                                                                                                                                                                                                   |
...
| `due-date`               | `none` \| `datetime` \| `str` \| `content`                                                   | An explicit due date, printed by `references.due-date()`: a `datetime` is the due date (BT-9) of the e-invoice, a text its payment terms (BT-20). See [Payment Terms and Instructions](../../e-invoicing.md#5-payment-terms-and-instructions).                                                                                                                                                            |
...
| `zugferd-strict`         | `bool`                                                                                       | Checks the e-invoice more thoroughly (default `false`): every invoice line of the written XML is compared with the invoice, and the sums of its amounts are checked like the official rules do. `--input zugferd-strict=true` turns it on for a compilation, e.g. in CI. See [The Strict Mode](../../e-invoicing.md#the-strict-mode).                                                                     |
