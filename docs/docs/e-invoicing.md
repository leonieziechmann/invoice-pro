...
Codes (currencies, countries, units, schemes of identifiers and electronic addresses, payment means and VAT exemption reasons) must be in the code lists that the validation of the profile applies, and the error names the rule of the validator that rejects the code: `"minimum"` and `"basic-wl"` apply the lists of Factur-X 1.0.07 alone (e.g. `FX-SCH-A-000040` for the currency), `"xrechnung"` both versions of the EN 16931 code lists alone (1.3.12 in Mustang, 1.3.16 in KoSIT; e.g. `BR-CL-04`), and `"basic"` and `"en16931"` all of them (`BR-CL-04`, or `FX-SCH-A-000040` for a code that only the Factur-X list lacks). So `"xrechnung"` accepts codes that the Factur-X lists lack, such as the electronic address schemes `0219` and `0220`, the Netherlands Antilles (`AN`) or the São Tomé dobra (`STD`), and `"minimum"` and `"basic-wl"` accept South Sudan (`SS`), which the EN 16931 lists lack. A code that the newest EN 16931 list has withdrawn is rejected even where the validation of the profile still accepts it (`IP-CODE-01`): a currency such as `BGN` in the profiles based on EN 16931, and the electronic address scheme `9901` in every profile. A code that only the newest list has, e.g. the Caribbean guilder `XCG` (2025), is reported as a code the validation of the profile does not know yet. Earlier versions applied every list in every profile, so they rejected these codes in XRechnung as well, and named the rules of EN 16931 (`BR-CL-*`) in `"minimum"` and `"basic-wl"`, whose validation does not have them.
...
| `IP-CODE-01`    | error   | A code that the newest EN 16931 code list (1.3.16) has withdrawn, which the validation of BASIC WL and BASIC still accepts: the currencies BGN and HRK (replaced by the euro) in BASIC, the electronic address scheme 9901 in both. A receiver that validates with the current list rejects the e-invoice, as KoSIT does in EN 16931 and XRechnung (`BR-CL-04`, `BR-CL-25`). |
...
| `IP-PAY-03`     | error   | Two kinds of payment means in `"basic-wl"` or `"basic"`, whose validation accepts them together, e.g. `bank-details` next to `card-payment`: an invoice states one payment means (BT-81), so that the buyer does not pay twice. The other profiles report them as `CII-SR-467` (or `BR-DE-23-b`, `BR-DE-24-b`).                                                              |
...
| `IP-PRINT-01`   | error   | A total the XML states (BT-109, BT-112) differs from the printed one, or its VAT breakdown (BT-116) does not add up to the printed total without VAT. A failure is a bug of invoice-pro.                                                                                                                                                                                     |
...
- **Electronic Addresses & EAS Routing (BT-34 / BT-49):** For routing across networks (such as Peppol), both parties need an electronic address. XRechnung requires them (`PEPPOL-EN16931-R020`, `PEPPOL-EN16931-R010`). EN 16931 leaves them optional, and its validation checks neither: in `"en16931"` a missing address is a warning of `invoice-pro`'s own (`IP-EADDR-01`, formerly reported under the Peppol ids), and the other profiles do not report it.
...
- **Custom / Dictionary Units:** If you have special or custom unit requirements, pass a dictionary containing both the display text and the official UN/ECE code:
  ...
  ```
- **Automatic Mapping:** A string that is exactly a unit code (e.g. `"H87"`) is used as is. Other strings are mapped by the unit names of every language of `invoice-pro`, the symbols of the `unit` module and common abbreviations, e.g. `"Std."` or `"hrs"` for hours, `"Tage"` for days, `"m²"` or `"qm"` for square metres, `"km"`, `"t"`, `"kWh"`, `"Stk."` for pieces, `"Seiten"` for pages or `"pauschal"` for a lump sum. A string `invoice-pro` does not know is an error (`IP-UNIT-02`, formerly reported as BR-CL-23, which the XML it would write, with the code `C62`, does not break) rather than a guess: pass the unit as a dictionary with its code, e.g. `(display: "Nacht", code: "C62")` for a number of nights. A code that is also a common German abbreviation of another unit (`"STK"` is the code of sticks, `"PAL"` of pascal; also `"FL"`, `"GL"` and `"KT"`) is taken as a code, with a warning (`IP-UNIT-01`).
...
In the profiles based on EN 16931, the currency must be in the code lists of both official validators (`BR-CL-04`): a currency that is newer than the list of one of them (e.g. `VES`) stops the e-invoice, and so does one that the current list has withdrawn (e.g. `BGN` and `HRK`, replaced by the euro; `IP-CODE-01` in `"basic"`, whose validation still accepts it). `"basic"` and `"en16931"` also apply the list of Factur-X (`FX-SCH-A-000040`), which lacks e.g. the São Tomé dobra of before 2018 (`STD`), which `"xrechnung"` accepts.
