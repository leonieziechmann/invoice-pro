...
- **Structure (G1):** every element is known to the profile's schema at its position and not marked as not used there by the Factur-X Schematron; the elements are in schema order and number (`maxOccurs` and the counts of the Schematron); every element the schema or the Schematron requires is there with its text (an element without text counts as missing), e.g. for `"xrechnung"` the city and post code of the addresses, the contact of the seller, the buyer reference and the rate of each VAT breakdown; every required attribute is there, and no other attribute.
