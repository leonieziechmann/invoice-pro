...
| `IP-CALC-01`    | error   | The parts of an allowance or charge per VAT category (BG-20, BG-21) do not add up to its printed amount, or one has the other sign, which the XML would state as a charge instead of an allowance (or the other way round). A failure is a bug of invoice-pro.                     |
