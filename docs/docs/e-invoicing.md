...
The validator checks the invoice data; a second check, the write guard, checks the XML itself while it is written, against tables compiled from the official artefacts of the profile (the Factur-X 1.0.07 XSD and Schematron, the CEN Schematron of EN 16931 and, for `"xrechnung"`, the XRechnung 3.0 Schematron). It guarantees for every e-invoice `invoice-pro` attaches:

- **Structure (G1):** every element is known to the profile's schema at its position and not marked as not used there by the Factur-X Schematron; the elements are in schema order and number (`maxOccurs` and the counts of the Schematron); every required element and attribute is there, and no other attribute.
...
| `IP-GUARD-04` | A required element that is missing.                                                                                     |
...
| `IP-GUARD-08` | A date in the format `102` with eight digits that name no day (e.g. `20260230`).                                        |
...
The guard checks what the schema and the code lists say about each element, not the business rules (sums, conditions between fields), which remain the validator's. It is stricter than the official validators in a few documented places: it rejects the elements the Factur-X Schematron marks as not used (Mustang ignores those reports), dates that name no day, and any element `invoice-pro` never writes; and where a rule of the CEN Schematron may be taken over by a rule of higher priority only under a condition on values, it applies the rule anyway.
