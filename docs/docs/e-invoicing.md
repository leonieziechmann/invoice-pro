...
Codes (currencies, countries, units, schemes of identifiers and electronic addresses, payment means and VAT exemption reasons) must be in the code list of every official validator of the profile: the lists of Factur-X 1.0.07 and, in the profiles based on EN 16931, both versions of the EN 16931 code lists (1.3.12 in Mustang, 1.3.16 in KoSIT). A code that only the newest list has, e.g. the Caribbean guilder `XCG` (2025), is reported as a code the validation of the profile does not know yet.
