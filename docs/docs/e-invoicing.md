...
Codes (currencies, countries, units, schemes of identifiers and electronic addresses, payment means and VAT exemption reasons) must be in every official code list, whatever the profile: in the lists of Factur-X 1.0.07 and in both versions of the EN 16931 code lists (1.3.12 in Mustang, 1.3.16 in KoSIT). This can be stricter than the validation of a profile, which may apply fewer of these lists (XRechnung, for example, is not validated with the Factur-X lists). Only a country or currency of `"minimum"` and `"basic-wl"`, whose validation applies the Factur-X lists alone, needs to be in the Factur-X list only (e.g. South Sudan, `SS`, which the EN 16931 list lacks). A code that only the newest list has, e.g. the Caribbean guilder `XCG` (2025), is reported as a code the validation of the profile does not know yet.
