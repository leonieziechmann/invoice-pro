...
2. **Code blocks with version numbers** (e.g., `@preview/invoice-pro:0.4.1`) must be flagged with the version so they can be updated during releases.
...
| :------------- | :-------------------------------------------------- | :------ | :-------------------- | :---- |
| `quick-glance` | Full invoice with items, discount, and bank details | `0.4.1` | `docs/intro-minimal/` |       |

### `getting-started.md`

| Code ID         | Description                                      | Version | Test                            | Notes                             |
| :-------------- | :----------------------------------------------- | :------ | :------------------------------ | :-------------------------------- |
| `import`        | Package import statement                         | `0.4.1` | —                               | Trivial one-liner, no test needed |
| `first-invoice` | Minimal invoice with items and tax configuration | `0.4.1` | `docs/getting-started-minimal/` |                                   |
...
| `dev-import` | Import snippet showing package injection | `0.4.1` | —    | Trivial snippet          |
...
| Code ID     | Description                                                       | Version | Test                        | Notes |
| :---------- | :---------------------------------------------------------------- | :------ | :-------------------------- | :---- |
| `blueprint` | Full architectural blueprint with items, payment, bank, signature | `0.4.1` | `docs/api-index-blueprint/` |       |
...
| `minimal-config`   | Minimal valid configuration with a single item | `0.4.1` | `docs/api-invoice-minimal/` |                                   |
...
| `apply-bulk-tax`       | Apply block wrapping items with shared lower tax rate | `0.4.1` | `docs/api-components-apply/` |                   |

### `api-reference/tax.md`

| Code ID          | Description                         | Version | Test | Notes                                   |
| :--------------- | :---------------------------------- | :------ | :--- | :-------------------------------------- |
| `reverse-charge` | Reverse-charge tax usage on an item | `0.4.1` | —    | Snippet only (no full document context) |
| `custom-tax`     | Custom tax category with `tax.new`  | `0.4.1` | —    | Snippet only (let binding)              |

### `api-reference/theme.md`

| Code ID           | Description                                          | Version | Test                      | Notes |
| :---------------- | :--------------------------------------------------- | :------ | :------------------------ | :---- |
| `din5008-example` | DIN-5008 theme with custom form, font, and hole-mark | `0.4.1` | `docs/api-theme-din5008/` |       |
| `blank-example`   | Blank theme with native Typst page setup             | `0.4.1` | `docs/api-theme-blank/`   |       |
...
| Code ID       | Description                                 | Version | Test | Notes                             |
| :------------ | :------------------------------------------ | :------ | :--- | :-------------------------------- |
| `pl-language` | Polish language dictionary definition       | `0.4.1` | —    | Part of multi-file locale example |
| `pl-region`   | Polish region builder function              | `0.4.1` | —    | Part of multi-file locale example |
| `pl-factory`  | Building locale with `build-locale` factory | `0.4.1` | —    | Part of multi-file locale example |
| `pl-usage`    | Using the custom locale in a document       | `0.4.1` | —    | Part of multi-file locale example |

### `api-reference/locale/base.md`

| Code ID           | Description                                     | Version | Test | Notes      |
| :---------------- | :---------------------------------------------- | :------ | :--- | :--------- |
| `schema-override` | Schema inspection and partial language override | `0.4.1` | —    | Needs test |
