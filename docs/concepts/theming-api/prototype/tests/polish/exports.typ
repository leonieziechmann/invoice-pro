// API hygiene: the exported names of every public module == the documented list
// (README §2.1, §2.4, §3.6, §8.1). A Typst module exports every top-level name,
// imports included, so an internal helper imported into a public module becomes
// API by accident; public modules therefore re-export by name only.
...
#metadata((
  theme: names(theme).len(),
  layout: names(theme.layout).len(),
  custom: names(theme.custom).len(),
  parts: names(theme.parts).len(),
)) <exports>
ok
