// Gallery: `compact` - an industrial wholesaler's collective invoice (Sammelrechnung)
// over three delivery notes, with seller item numbers, trade units and a volume
// discount. Default layout: layout auto (sender region de -> a4-dense).
// Item numbers are the seller's (item-id: (seller: ..) -> ram:SellerAssignedID in
// the XML; a plain string is a standard id such as a GTIN). The compact table reads
// seller, standard or buyer.
//   typst compile --root . tests/gallery/compact.typ out/gallery/compact-{p}.png --ppi 110
//   --input layout=din-5008-a|us-letter-10|..   --input lang=en   --input zugferd=1
//   --input logo=image (UA test: an image logo with alt text)   --input check=1
//   --input n=<k>  items in the last delivery note run up to index k (default 52)
//   --input seed=<hex>  another brand colour
#import "/src/lib.typ": *

#let lay = sys.inputs.at("layout", default: none)
#let seed = sys.inputs.at("seed", default: none)
#let en = sys.inputs.at("lang", default: "de") == "en"
#let strict = sys.inputs.at("check", default: "0") == "1"
...
#let logo = if sys.inputs.at("logo", default: "mark") == "image" {
  image("/tests/doc/logo.svg", alt: "Nordwerk Industriebedarf GmbH")
} else { mark }
...
#let row(i, q: auto) = {
  ...
  item(name, item-id: (seller: sku), unit: u, price: p, quantity: if q == auto {
    qty.at(calc.rem(i, qty.len()))
  } else { q })
}
...
#show: invoice.with(
  theme: theme.compact.with(
    ...
    if strict { theme.custom.checks(min-contrast: 4.5) },
...
#line-items[
  ...
  #group([Lieferschein LS-26-5671 vom 26.09.2026])[
    #for i in range(34, int(sys.inputs.at("n", default: "52"))) { row(i) }
  ]
