...
#let render-table(
  ...
  let build-item-rows(
    ...
    let left-spacer = if keep-together { ls } else { left-spacer }
    let cap-span = if keep-together { total-cols + 1 } else { total-cols + 2 }
    let cap-stroke(side) = if keep-together {
  ...
  let build-group-footer-rows(group, bind: false, lead: ()) = {
    let rows = ()
    let cell-inset = styles.cell-inset
    let item-inset = styles.item-inset
    let item-stroke = styles.item-stroke
    ...
    let span = bind or keep-together
    let ls = "__left-spacer__"
  ...
  table(
    ...
    ..if not bound { (table.hline(stroke: styles.stroke-table-bottom),) },
  )
}
