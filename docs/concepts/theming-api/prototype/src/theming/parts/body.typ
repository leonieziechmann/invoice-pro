...
// The prototype adapts the existing generic renderers (M3 replaces them).
//
...
/// Set by the built-in items-table when it placed `view.tail` (the totals).
#let _tail-bound = state("ip-tail-bound", false)
...
#let items-table(ctx, view) = {
  ...
  if view.at("tail", default: none) != none { _tail-bound.update(true) }
}
