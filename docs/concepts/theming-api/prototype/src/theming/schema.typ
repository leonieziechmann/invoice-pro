...
// Rule: a token is frozen only if a built-in part reads it (CI mutation test,
// scripts/mutation.sh). `fonts.regulated` arrives with reserved zones (0.5.x).
#let token-schema = (
  colors: (
...
#let option-schema = (
  ...
  items-table: (
    // (odd, even) fills, each none | colour | derivation, OR a callback (index) => fill (#33)
