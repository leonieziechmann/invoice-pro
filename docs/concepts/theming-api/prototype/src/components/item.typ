    ...
    scope: ctx => loom.mutator.batch(ctx, {
      ...
      derive(
        ...
        unit-logic.resolve(
          unit,
          ctx.locale,
          // the item's own quantity (derive() above does not update `ctx`), so
          // plural unit names follow it: "2,5 Stunden", "1 Stunde"
          quantity: if quantity == auto {
            ctx.at("quantity", default: decimal("1"))
          } else { coercion.to-decimal(quantity) },
          default: m-unit.pc,
        ),
      )
