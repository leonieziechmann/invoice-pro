...
#let payment-goal(
  ...
  managed-motif(
    ...
    draw: (ctx, _, view, ..) => {
      ...
      // `text-due` with prepayments), so that is the sentence of the payment
      // means here, followed by the notes of the cash discounts.
      let ctx = ctx
      ctx.locale.strings.payment = goal-strings(
