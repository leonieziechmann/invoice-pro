// Printed = written: the invariants that the e-invoice states the amounts
// and quantities the invoice computed and prints (concept 4.2). They compare
// the data model of the e-invoice, whose XML the write guard compares with
// it once more (../guard/roundtrip.typ), with the computed invoice it is a
// projection of: the items, allowances, charges and VAT groups of the line
...
// value for the print and another one for the e-invoice), never a mistake
// in the invoice data, so every finding is an error.
//
