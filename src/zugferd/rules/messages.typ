// The messages and hints of the rules of the registry, and `diagnostics`,
// which turns the findings of the checks into diagnostics with the metadata
// of registry.json. engine.typ loads this module only when a check fails:
// the texts are the largest part of the rules. See engine.typ for the
// registry and the form of a finding.
//
...
/// Turns the findings of the checks (see engine.typ) into diagnostics,
/// errors first. A finding of a key without an entry in the registry, of an
/// id or a level the entry does not have, or without a message stops the
/// compilation: every diagnostic comes from the registry.
///
/// -> array
#let diagnostics(findings) = {
  ...
  for f in findings {
    ...
    let build = messages.at(f.key, default: none)
