...
def validator_lists(compilers, names):
    ...
    for name, (path, attr, factur_x) in sorted(VALIDATOR_LISTS.items()):
        ...
        lists, every = at["en16931"]
        entry = {"every": names.name(every)}
        if at["basic"] is not None and at["basic"][1] != every:
...
OUTPUT_FILES = ["lists.typ"] + [f"{p}.json" for p in PROFILES]
...
def is_generated(path):
    ...
    that starts with its HEADER, or JSON with its notice."""
    if path.suffix == ".typ":
        return path.read_text(encoding="utf-8").startswith(HEADER)
    if path.suffix == ".json":
        return f'"generated":{json_value(JSON_NOTICE)}' in path.read_text(encoding="utf-8")
    return False
