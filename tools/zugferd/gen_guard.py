...
serializer src/zugferd/guard/write.typ reads it and describes the format):
  ...
  src/zugferd/guard/lists.typ      the code lists, and the lists of the
                                   validator (`validator`, see
...
def typ_value(value):
    ...
    if isinstance(value, str):
        return typ_str(value)
    raise GenError(f"no Typst literal for {value!r}")
...
def wrap(prefix, items, suffix, indent, width=80):
    ...
    if len(one) + indent <= width and "\n" not in one:
        return one
    pad = " " * (indent + 2)
    return prefix + "\n" + "".join(f"{pad}{item},\n" for item in items) + " " * indent + suffix


def import_line(path, names, width=80):
    """An import of `names` from `path` in typstyle's layout: one line when
    it fits, else the names in parentheses, as many per line as fit."""
    one = f'#import "{path}": ' + ", ".join(names)
    if len(one) <= width:
        return one
    lines, line = [], []
    for name in names:
        if line and len("  " + ", ".join(line + [name]) + ",") > width:
...
def emit_lists(names, validator=None):
    ...
    out = [
        ...
        "",
        "#let _derive(list, add: (), remove: ()) = {",
        '  for code in remove { list = list.replace(" " + code + " ", " ") }',
        '  if add.len() > 0 { list += add.join(" ") + " " }',
        "  list",
        "}",
        "",
    ]
...
def emit_profile(profile, nodes, names, digests):
    """src/zugferd/guard/<profile>.typ: the nodes of one profile."""
    ...
    intro = (
        f"The nodes of the profile `{profile}` (node 0: the root element), compiled from the "
        + ", ".join(sources)
...
def emit_high(high):
    return "none" if high == UNBOUNDED else str(high)
...
def emit_target(target, nodes):
    ...
    kind = target[0]
    if kind == "N":
        return str(nodes.ids[target[1]])
    if kind == "D":
        return str(nodes.ids[target])
    return typ_str(target[1])
...
def emit_dispatch(key, nodes):
    ...
    fields = [
        "d: " + typ_array([typ_str(s) for s in disc]),
        "m: " + typ_dict((v, emit_target(t, nodes)) for v, t in values),
        "o: " + emit_target(other, nodes),
        "k: " + typ_dict((v, typ_array([str(hi), typ_value(hr)])) for v, (lo, hi, lr, hr) in limits if hi != UNBOUNDED),
    ]
    return "(" + ", ".join(fields) + ")"
...
def emit_action(target, nodes, names, imports):
    ...
    this fast class (see `leaf_fast`), or a plain text in this code list (a
    variable of lists.typ, added to `imports`); `none` when every value
    takes the general path (a leaf with further checks, a family element)."""
    if target[0] != "N":
        return "none"
    key = target[1]
    if key[0] == "C":
        return str(nodes.ids[key])
    ref = leaf_list(*key[1:])
    if ref is not None:
        imports.add(names.name(ref.codes))
        return names.name(ref.codes)
    return typ_value(leaf_fast(*key[1:]))


def emit_list(ref, names):
    if ref is None:
        return "none"
    exceptions = typ_dict((c, typ_str(r)) for c, r in ref.exceptions) if ref.exceptions else "none"
    return typ_array([typ_str(names.name(ref.codes)), typ_str(ref.rule), exceptions, typ_value(ref.casefold)])


def emit_node(key, nodes, names, imports):
    """A leaf as its kind, or as an array (kind, attributes, list, prefix
    ...
    if key[0] == "L":
        ...
        if not attrs and text is None and prefix is None and fraction is None and date is None:
            return typ_str(kind)  # a leaf without further checks: its kind
        items = [typ_str(kind)]
        if attrs:
            pairs = []
            for name, required, forbidden, lst in attrs:
                req = "false" if required is None else "true" if required == "XSD" else typ_str(required)
                pairs.append((name, typ_array([req, typ_value(forbidden), emit_list(lst, names)])))
            items.append(typ_dict(pairs))
        else:
            items.append("none")
        items.append(emit_list(text, names))
        if prefix:
            ref, when = prefix
            items.append(typ_array([emit_list(ref, names), "none" if when is None else typ_array([typ_str(w) for w in when])]))
        else:
            items.append("none")
        items.append("none" if fraction is None else typ_array([str(fraction[0]), typ_value(fraction[1]), typ_value(fraction[2])]))
    ...
    for tag, index, low, high, target, low_rule, high_rule in children:
        if high == 0:
            ...
            specs.append(f"{typ_str(tag)}: {typ_array([typ_str(high_rule or target[1])])}")
            continue
        parts = [emit_action(target, nodes, names, imports), str(index), str(low), emit_high(high), emit_target(target, nodes)]
        # The rules of the minimum and the maximum, as far as there are any.
        if high_rule:
            parts += [typ_value(low_rule), typ_value(high_rule)]
        elif low_rule:
            parts.append(typ_value(low_rule))
        specs.append(f"{typ_str(tag)}: {typ_array(parts)}")
    fields = [f"n: {sum(1 for c in children if c[2] > 0)}"]
    extras = []
    if empty not in (None, "ok"):
        extras.append(f"e: {typ_str(empty)}")
    minima = [
        typ_array([typ_str(tag), typ_str(v), str(lo), typ_value(lr)])
    ...
    if minima:
        extras.append("v: " + typ_array(minima))
    if aggregates:
        extras.append("g: " + typ_array([
            typ_array([typ_array([typ_str(t) for t in path]), str(lo), emit_high(hi), typ_str(rule)])
            for path, lo, hi, rule in aggregates
        ]))
    if anyof:
        extras.append("y: " + typ_array([typ_array([typ_array([typ_str(t) for t in tags]), typ_str(rule)]) for tags, rule in anyof]))
    if exclusive:
        extras.append("x: " + typ_array([typ_array([typ_array([typ_str(t) for t in tags]), typ_str(rule)]) for tags, rule in exclusive]))
    if xref:
        extras.append("r: " + typ_array([
...
OUTPUT_FILES = ["lists.typ"] + [f"{p}.typ" for p in PROFILES]
...
def generate(jar_path, out_dir, pinned=True, builder=BUILDER, kosit_config=None):
    ...
    for p in PROFILES:
        files[f"{p}.typ"] = emit_profile(p, nodes[p], names, jar.digests)
