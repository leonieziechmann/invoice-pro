...
The model is a projection of the computed invoice; `src/zugferd/rules/equivalence.typ` checks that it states what the invoice prints (`IP-PRINT-01`, `IP-CALC-01`, `IP-CALC-02`, and `PEPPOL-EN16931-R120` of XRechnung): in one pass for every invoice, and in detail (`equivalence-detail.typ`, which also reports the findings) when a value differs, with gross prices, with allowances or charges, and for an XRechnung in a currency without cents. `tests/zugferd/equivalence/test.typ` changes the model or the computed invoice after the computation and expects each rule to report it.
