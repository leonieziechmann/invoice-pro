...
// The serif letterhead (prototype tests/polish/arrange.typ, letterhead.typ,
// scripts/checks-fix-polish.sh):
// 1. the arrangement fits its box and never squeezes the logo column (a
//    squeezed column cut prestige's on-dark plate into white bars on
...
// 2. stress renders: prestige and elegant on every layout with a long sender
//    name and a wide or a tall logo on the on-dark plate (2 x 16 x 2 invoices).
...
#let logos = (
  wide: theme.custom.logo(image: logo-img),
  tall: theme.custom.logo(image: tall-mark, height: 18mm),
)
#let stress = (
  ("prestige", "elegant")
    .map(look => layouts.map(lay => ("wide", "tall").map(lg => (
      look,
      lay,
      lg,
    ))))
...
#for (look, lay, lg) in stress {
  case(
    look + "/" + lay + "/" + lg,
    theme: preset-of(look).with(logos.at(lg), layout: layout-of(lay)),
    locale: test-locale,
    ..party,
    sender: party.sender + (name: "Grand Hotel Bellevue Palace Bern AG"),
    body(n: 4),
  )
}
#close-cases()
#context assert.eq(spans().len(), stress.len())
