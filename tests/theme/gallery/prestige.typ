...
//   layout: <name>   another layout (default: the preset's choice)
//   logo: "light" (default): the dark mark plus a champagne on-dark version;
//         "plate": only the dark mark (it gets the light plate on the band);
//         "none": no logo
//   extra: <n>       more in-room dining lines (a long folio closes with a signature)
#import "/src/lib.typ": *
...
#let gallery(
  ...
  let logo = if logo == "light" {
    theme.custom.logo(image: mark, height: 13mm, on-dark: image(
      "pa-mark-light.svg",
      alt: "Palais Aurelia",
    ))
  } else if logo == "plate" { theme.custom.logo(image: mark, height: 13mm) }
  ...
  invoice(
    ...
    sender: (
      ...
      extra: (
        Phone: "+43 1 512 94 00",
        "E-Mail": "reception@palais-aurelia.at",
      ),
    ),
    recipient: (
