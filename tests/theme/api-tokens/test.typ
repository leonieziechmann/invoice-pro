// api-tokens (prototype tests/api-tokens.typ, scripts/checks-api-tokens.sh): the
// frozen tokens, declared contrast pairs, theme label strings in the locales,
// per-part option metadata, and the misuse/strict probes of checks.pairs.
#import "/src/lib.typ": *
...
// --- 5. misuse and strict probes of checks.pairs (the pair is named) -------------------
#let probes = (
  (
    () => theme.custom.checks(pairs: (bad: t => t.colors.primary)),
    none,
    "panicked with: "
      + repr(
        "theme::checks::pairs::bad must be a derivation `t => (foreground, background)` or a pair of colours, found rgb(\"#1f2937\") (returned by the derivation)",
      ),
  ),
  (
    () => theme.custom.checks(pairs: (lit: (red, blue, green))),
    none,
    "panicked with: "
      + repr(
        "theme::checks::pairs::lit must be a derivation `t => (foreground, background)` or a pair of colours, found (rgb(\"#ff4136\"), rgb(\"#0074d9\"), rgb(\"#2ecc40\"))",
      ),
  ),
  (
    () => ((checks: (pairs: (t => (red, blue),))),),
    none,
    "panicked with: "
      + repr(
        "variable `theme::checks::pairs`(((..) => ..,)) must be of dictionary",
      ),
  ),
  (
    () => theme.custom.checks(pairs: (t => (red, blue),)),
    none,
    "assertion failed: variable `theme::custom::checks::pairs`(((..) => ..,)) must be of dictionary: (name: t => (foreground, background))",
  ),
  (
    () => theme.custom.checks(min-contrast: 4.5, pairs: (
      disc: t => (t.colors.tint, t.colors.background),
    )),
    "strict",
    "panicked with: "
      + repr(
        "theme: checks::pairs::disc (#e2e8f0 on #ffffff) has contrast 1.23:1, below checks.min-contrast 4.5:1",
      ),
  ),
)
#for (patch, level, message) in probes {
  assert.eq(
    catch(() => theme.resolve(theme.classic.with(patch()), validation: level)),
    message,
  )
}
