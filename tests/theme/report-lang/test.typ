...
// The draft report speaks the document's language (prototype
// tests/polish/report-lang.typ, scripts/checks-fix-polish.sh): every issue that
// can reach the report has a text in de, en, fr, it and es (identifiers stay
// code), and a real draft with an invalid IBAN, a logo without alt text, a fine
// size under 6 pt and a failed contrast pair renders its report in each of the
// five languages (five invoices of this document, each with its own report).
#import "/src/lib.typ": *
...
// every `key:` passed to issue(..) in src/ (the prototype's
// scripts/checks-fix-polish.sh grepped src/ and compared it with this list; a
// test cannot read src/, so the keys are pinned against the locales here)
#let issue-keys = (
  "contrast",
  "cmyk",
  "envelope",
...
// every `key:` passed to an issue in src/ has texts (the prototype grepped
// src/components, src/theming, src/validation and src/zugferd; a file added
// there must be added to this list)
#let src-files = (
  "/src/components/bank-details.typ",
  "/src/components/bundle.typ",
  "/src/components/dynamic.typ",
  "/src/components/group.typ",
  "/src/components/item.typ",
  "/src/components/line-items.typ",
  "/src/components/modifier.typ",
  "/src/components/payment-terms.typ",
  "/src/components/prepayment.typ",
  "/src/components/root.typ",
  "/src/components/signature.typ",
  "/src/theming/access.typ",
  "/src/theming/build.typ",
  "/src/theming/color.typ",
  "/src/theming/custom.typ",
  "/src/theming/data.typ",
  "/src/theming/frame.typ",
  "/src/theming/layout-ops.typ",
  "/src/theming/layouts.typ",
  "/src/theming/looks/bold.typ",
  "/src/theming/looks/boxed.typ",
  "/src/theming/looks/compact.typ",
  "/src/theming/looks/corporate.typ",
  "/src/theming/looks/elegant.typ",
  "/src/theming/looks/kit.typ",
  "/src/theming/looks/prestige.typ",
  "/src/theming/looks/serif.typ",
  "/src/theming/looks/soft.typ",
  "/src/theming/looks/technical.typ",
  "/src/theming/parts/body.typ",
  "/src/theming/parts/defaults.typ",
  "/src/theming/parts/frame.typ",
  "/src/theming/presets.typ",
  "/src/theming/proof.typ",
  "/src/theming/resolve.typ",
  "/src/theming/schema.typ",
  "/src/theming/scope.typ",
  "/src/theming/validate.typ",
  "/src/validation/data.typ",
  "/src/validation/issue.typ",
  "/src/validation/render.typ",
  "/src/zugferd/build.typ",
  "/src/zugferd/xml.typ",
)
#let src-keys = (
  src-files
    .map(f => read(f)
      .matches(regex("key: \"([a-z-]+)\""))
      .map(m => m.captures.first()))
    .flatten()
    .dedup()
    .sorted()
)
#assert.eq(src-keys, issue-keys.sorted())
...
#import "/tests/theme/harness.typ": case, close-cases, issues-of
#let locales = (
  de: locale.de-de,
  en: locale.en-de,
  fr: locale.fr-de,
...
#for (lang, loc) in locales {
  case(
    lang,
    theme: theme.classic.with(
      theme.custom.logo(image: image("/tests/theme/gallery/pa-mark.svg")),
...
#context for (lang, strings) in all-langs {
  if lang == "base" { continue }
  let strings = strings.validation
  let issues = issues-of(lang)
  let ids = issues.map(x => x.id)
  for want in ("iban", "lint/logo-alt", "lint/fine-size") {
    assert(
      want in ids,
      message: lang + ": expected issue " + want + " in " + repr(ids),
    )
  }
  assert(
    ids.any(i => i.starts-with("lint/contrast-")),
    message: lang + ": " + repr(ids),
  )
  for x in issues {
  ...
  for (id, needle) in ("iban", "lint/logo-alt", "lint/fine-size").zip(
    needles.at(lang),
  ) {
    assert(
